...
def map_image_to_costs(image,binary,Obstcles):
    """
    ...
    :param image, binary which will be used as exits (distance=0), obscles which will have distance set to inf
    ...
    obstacles = np.where(Obstcles>0)#[] #np.where(np.abs(data[:, :, 0] + data[:, :, 2]) < 1. / 256)
    # Convert image to greyscale
    grey_scales = data#np.dot(data[..., :3], [0.299, 0.587, 0.114])
    # Boolean index array for places without exits and obstacles
    space = np.ones(grey_scales.shape, dtype=np.bool_)
...
def _propagate_distance(cell, costs, wdt_field):
    ...
    a = 1. / (hor_cost+1e-17) ** 2 + 1. / (ver_cost+1e-17) ** 2
    b = -2 * (hor_pot / (hor_cost+1e-17) ** 2 + ver_pot / (ver_cost+1e-17) ** 2)
    c = (hor_pot / (hor_cost+1e-17)) ** 2 + (ver_pot / (ver_cost+1e-17)) ** 2 - 1

    D = b ** 2 - 4 * a * c
    # Largest root represents upwind approximation
    if not (D<0):
        x_high = (2 * c) / (-b - math.sqrt(D)+1e-17)
    else:
        x_high=0
    return x_high
